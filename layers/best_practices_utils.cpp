...
bool BestPractices::PreCallValidateCreateImage(VkDevice device, const VkImageCreateInfo* pCreateInfo,
    ...
    if (pCreateInfo->samples > kMaxEfficientSamplesArm) {
        skip |=
            !disabled.vendor_specific_arm &&
            LogPerformanceWarning(device, kVUID_ArmBestPractices_CreateImage_TooLargeSampleCount,
                                  "vkCreateImage(): Trying to create an image with %u samples. "
    ...
    if (!disabled.vendor_specific_arm && pCreateInfo->samples > VK_SAMPLE_COUNT_1_BIT &&
        !(pCreateInfo->usage & VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT)) {
        skip |= LogPerformanceWarning(
            device, kVUID_ArmBestPractices_CreateImage_NonTransientMSImage,
            "vkCreateImage(): Trying to create a multisampled image, but createInfo.usage did not have "
        ...
        for (uint32_t j = 0; j < pCreateInfo->pColorBlendState->attachmentCount; j++) {
            ...
            if (att != VK_ATTACHMENT_UNUSED && blend_att.blendEnable && blend_att.colorWriteMask) {
                if (!FormatHasFullThroughputBlendingArm(rp_state->createInfo.pAttachments[att].format)) {
                    skip |= LogPerformanceWarning(
                        device, kVUID_ArmBestPractices_CreatePipelines_MultisampledBlending,
                        "vkCreateGraphicsPipelines() - createInfo #%u: Pipeline is multisampled and color attachment #%u makes use "
...
bool BestPractices::PreCallValidateCreateGraphicsPipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t createInfoCount,
    ...
    for (uint32_t i = 0; i < createInfoCount; i++) {
        ...
        skip |= !disabled.vendor_specific_arm && ValidateMultisampledBlendingArm(createInfoCount, pCreateInfos);
    }
...
bool BestPractices::PreCallValidateBeginCommandBuffer(VkCommandBuffer commandBuffer,
    ...
    if (!(pBeginInfo->flags & VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT)) {
        skip |= !disabled.vendor_specific_arm &&
                LogPerformanceWarning(device, kVUID_ArmBestPractices_BeginCommandBuffer_OneTimeSubmit,
                                      "vkBeginCommandBuffer(): VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT is not set. "
            ...
            if (attachmentNeedsReadback) {
                skip |= !disabled.vendor_specific_arm &&
                        LogPerformanceWarning(
                            device, kVUID_ArmBestPractices_BeginRenderPass_AttachmentNeedsReadback,
                            "Attachment #%u in render pass has begun with VK_ATTACHMENT_LOAD_OP_LOAD.\n"
                            ...
                            att, pRenderPassBegin->renderArea.extent.width * pRenderPassBegin->renderArea.extent.height,
...
bool BestPractices::PreCallValidateCmdDrawIndexed(VkCommandBuffer commandBuffer, uint32_t indexCount, uint32_t instanceCount,
    ...
    if ((indexCount * instanceCount) <= kSmallIndexedDrawcallIndices &&
        ...
        skip |= !disabled.vendor_specific_arm &&
                LogPerformanceWarning(device, kVUID_ArmBestPractices_CmdDrawIndexed_ManySmallIndexedDrawcalls,
                                      ...
                                      kMaxSmallIndexedDrawcalls, kSmallIndexedDrawcallIndices);
    }
...
bool BestPractices::PreCallValidateCmdResolveImage(VkCommandBuffer commandBuffer, VkImage srcImage, VkImageLayout srcImageLayout,
    ...
    skip |=
        !disabled.vendor_specific_arm &&
        LogPerformanceWarning(device, kVUID_ArmBestPractices_CmdResolveImage_ResolvingImage,
...
bool BestPractices::PreCallValidateCreateSampler(VkDevice device, const VkSamplerCreateInfo* pCreateInfo,
    ...
    if (!disabled.vendor_specific_arm) {
        if ((pCreateInfo->addressModeU != pCreateInfo->addressModeV) || (pCreateInfo->addressModeV != pCreateInfo->addressModeW)) {
            skip |=
                LogPerformanceWarning(device, kVUID_ArmBestPractices_CreateSampler_DifferentWrappingModes,
        ...
        if ((pCreateInfo->minLod != 0.0f) || (pCreateInfo->maxLod < VK_LOD_CLAMP_NONE)) {
            skip |= LogPerformanceWarning(
                device, kVUID_ArmBestPractices_CreateSampler_LodClamping,
                "Creating a sampler object with LOD clamping (minLod = %f, maxLod = %f). This may cause reduced performance. "
        ...
        if (pCreateInfo->mipLodBias != 0.0f) {
            skip |= LogPerformanceWarning(
                device, kVUID_ArmBestPractices_CreateSampler_LodBias,
                "Creating a sampler object with LOD bias != 0.0 (%f). This will lead to less efficient descriptors "
                "being created and may cause reduced performance.",
        ...
        if ((pCreateInfo->addressModeU == VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER ||
            ...
            skip |= LogPerformanceWarning(
                device, kVUID_ArmBestPractices_CreateSampler_BorderClampColor,
                "Creating a sampler object with border clamping and borderColor != VK_BORDER_COLOR_FLOAT_TRANSPARENT_BLACK. "
        ...
        if (pCreateInfo->unnormalizedCoordinates) {
            skip |=
                LogPerformanceWarning(device, kVUID_ArmBestPractices_CreateSampler_UnnormalizedCoordinates,
                                      "Creating a sampler object with unnormalized coordinates. This will lead to less efficient "
                                      "descriptors being created and may cause reduced performance.");
        }

        if (pCreateInfo->anisotropyEnable) {
            skip |= LogPerformanceWarning(
                device, kVUID_ArmBestPractices_CreateSampler_Anisotropy,
                "Creating a sampler object with anisotropy. This will lead to less efficient descriptors being created "
