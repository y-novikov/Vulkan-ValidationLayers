...
TEST_F(VkLayerTest, NonSemanticInfoEnabled) {
   ...
   %non_semantic = OpExtInstImport "NonSemantic.Validation.Test"
    ...
    pipe.pipeline_layout_ = VkPipelineLayoutObj(m_device, {});
    m_errorMonitor->SetDesiredFailureMsg(VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                         "Shader requires extension " VK_KHR_SHADER_NON_SEMANTIC_INFO_EXTENSION_NAME);
    pipe.CreateComputePipeline();
    m_errorMonitor->VerifyFound();
}
...
TEST_F(VkLayerTest, ValidateDeviceFeatureShaderArrayDynamicIndexing) {
    ...
    std::string fsSource =
        ...
        "layout(set=0, binding=5) buffer foo2 { int x; } sb5[2];\n"
    ...
    if (m_device->props.apiVersion >= VK_API_VERSION_1_2) {
        fsSource.append(
            ...
            "layout(set = 0, binding = 10, rgba8) uniform imageBuffer stb10[2];\n");
    }
    fsSource.append(
        "layout(location = 0) out vec4 uFragColor;\n"
        "void main(){\n"
